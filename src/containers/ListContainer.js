...
const ListContainer = () => {
	...
	const onDragEnd = (result) => {
		...
		if (type === 'task') {
			...
			let start = lists.find(
				(list) => list.id === parseInt(source.droppableId)
			);
			let finish = lists.find(
			...
			if (start === finish) {
				...
				tasks.splice(newPosition, 0, ...startTask); // are these lines being used still?
				updateTaskOrderAfterDnd(newPosition, finishID, draggedTaskID);
			}

			if (start !== finish) {
				let dragged_task = start.tasks.find(
	...
	const moveTaskToDifList = (startID, finishID, newPosition, id) => {
		...
		fetch(
			`${url}/users/${currentUser.id}/lists/${startID}/tasks/${id}/move_task_to_new_list`,
			options
		)
			.then((r) => r.json())
			.then((reorderedData) => {
				...
				let listChanges = lists.map((list) =>
					list.id === startID
						? { ...list, tasks: startTasks }
						: list.id === finishID
						? { ...list, tasks: finishTasks }
						: list
				);
	...
	const updateListOrderAfterDnd = (newPosition, listID) => {
		...
		fetch(
			`${url}/users/${currentUser.id}/lists/${listID}/update_order`,
			options
		)
			.then((r) => r.json())
			.then(setLists);
	...
	const updateTaskOrderAfterDnd = (newPosition, finishID, taskID) => {
		...
		fetch(
			`${url}/users/${currentUser.id}/lists/${finishID}/tasks/${taskID}/update_order`,
			options
		)
			.then((r) => r.json())
			.then((updatedTasks) => {
				setLists(
					lists.map((list) =>
						list.id === finishID ? { ...list, tasks: updatedTasks } : list
					)
				);
			});
	};
